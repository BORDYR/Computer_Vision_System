...
from tkinter import *
import ObjectRecognizer, MySerialRobot, circlerecognizer
...
class RobotConnection(serial.Serial):
	...
	def hand_open(self):
		self.write('GO'.encode())

	def hand_close(self):
		self.write('GC'.encode('utf-16'))
...
stopbits: {self.robot._stopbits}''').grid(row=0, column=0)
	...
	def grab_objects(self):
		if not self.detectedObjects:
			...
			Label(self.frame, text=f'{len(self.detectedObjects)} objects available: {self.detectedObjects}').grid(row=2, column=2)
