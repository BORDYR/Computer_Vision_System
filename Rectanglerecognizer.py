...
def is_on_border(img_shape, contour, is_source=False):
    borders_x = set([i for i in range(int(img_shape[1] * 0.98), img_shape[1] + 1)])
    borders_y = set([i for i in range(int(img_shape[0] * 0.98), img_shape[0] + 1)])
    borders_x.update([i for i in range(0, int(img_shape[1] * 0.02) + 1)])
    borders_y.update([i for i in range(0, int(img_shape[0] * 0.02) + 1)])
    x_val = set()
    y_val = set()
...
def get_target_center(gray_image):
    ...
    return find_contour(res_img, 40000)
...
def get_resistor_center(gray_image):
    ...
    return find_contour(res_img, 30000)
...
def show_result_img(image, boxes=(), centers=()):
    ...
    for box in boxes:
        offset += 150
        cv.drawContours(img, [box], 0, (255, offset, offset), 2)
    offset = 0
    for center in centers:
        offset += 150
        cv.circle(img, tuple(map(lambda x: int(x), center)), 5, (255, offset, offset))
...
def get_image_with_realogram(img):
    ...
    if type(box_target) is int:
        return 0
    #box_mini, center_resistor = get_resistor_center(gray_image)
    return show_result_img(img, [box_target], [center_target])